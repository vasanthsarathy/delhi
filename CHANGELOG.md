...
- The binary package is renamed `delhi-cli` → **`delhi`**, so the name you install matches
  the binary you get. The old name was never published, so nothing depends on it.

### Added

- A documentation site at <https://vsarathy.com/delhi/> — a landing page and a fourteen-
  chapter book. Four chapters introduce epistemic logic assuming no background; then a
  tutorial that builds a second-order false belief from an empty file, reference chapters,
  and where delhi sits against DEL, mA*, EFP and PDKB.
- Tagging a release now publishes to crates.io as well as building binaries.

### Fixed

- **`delhi-gui` could not have been installed from crates.io.** `builtin.rs` reached
  outside the crate with `include_str!("../../../examples/…")`, and cargo cannot package
  files from beyond a crate root — it would have published cleanly and then failed to
  compile for everyone. The sources are inlined instead, generated by
  `tools/bundle-examples.sh`, with a test asserting they match `examples/` byte for byte.
  Two further escaping includes in `#[cfg(test)]` blocks are fixed the same way.
- `delhi-lang` and `delhi-mb` exclude `tests/` from their published tarballs rather than
  shipping integration tests that cannot compile away from the repository.
