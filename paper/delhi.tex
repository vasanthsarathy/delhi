...
\subsection{The capability this paper is about}
\label{sec:running}

Rather than motivate the formalism abstractly, we fix one capability and one scenario, and
keep both for the rest of the paper. \Cref{sec:motivating} works through what existing
approaches can and cannot say about it; every later section returns to it.

\begin{quote}
\textbf{The capability.} An assistant notices that a person holds a \emph{false belief}
bearing on their work. It must decide whether to intervene, and how.
\end{quote}

That decision is harder than it sounds, because doing it well requires four things at once.
The assistant must represent the person's belief \emph{separately from the fact}, or there is
no mistake to notice. It must judge whether the person \emph{would find out anyway}, since
intervening is pointless if so and urgent if not. It must know what \emph{telling} someone
achieves as against \emph{showing} them, because a report can be doubted and a demonstration
cannot. And it must know \emph{who else is in the room}, since correcting someone privately
and correcting them in front of a colleague are different acts.

\begin{quote}
\textbf{The scenario.} A shift handover. A dose of medication was \textbf{not} given. The
nurse believes it was --- she was interrupted and assumes the handover covered it. A robot
assistant observed that it was not. A doctor is present but has no idea either way, and may
be paged away at any moment.
\end{quote}

\Cref{lst:handover} is the whole domain. Notice its goal, which is the crux of the whole
paper:
...
Not \emph{the nurse believes the truth} and not \emph{the nurse knows the truth}, but
\emph{the nurse believes the truth in a way that will survive}: no further true information
will unsettle it. That is \emph{safe belief} (\Cref{sec:safe}), and it is the right success
criterion for the capability, because a merely-held belief can be undone by the next thing
the nurse overhears. It is also a goal that cannot be \emph{written down} in a formalism
whose only attitudes are knowledge and belief --- which is where \Cref{sec:motivating}
begins.
...
\section{What existing approaches can say about it}
\label{sec:motivating}

We now walk the scenario past the main families of formalism. The aim is not to score points
--- each family made a deliberate trade and got something in return --- but to locate
precisely where each one stops being able to express the problem, because those stopping
points are what determined \delhi's design.
...
every action they write. \textbf{Stops at:} step one. The mistake is not representable.
...
\textbf{Stops at:} false belief. This is where $m\mathcal{A}^*$ \citep{baral} and the EFP
...
Two problems remain, and both bear directly on the capability.
...
\textbf{Stops at:} nothing, in principle. In practice: the event models are hand-built, and
plan existence is undecidable in the general setting \citep{bolander2011}.
...
\textbf{Stops at:} the depth bound, and the absence of an operator for stability.

\subsection{What the scenario demands}
\label{sec:demands}

Reading the failures back gives a specification, and it is the specification \delhi{} was
built against.

\begin{enumerate}
  \item \textbf{Belief that can be false, with revision that preserves everything else.}
  ...
  \item \textbf{Safe belief in the object language.} The success criterion is stability, not
  ...
  \item \textbf{Event models derived from declared observability.} The modeller should write
  ...
  \item \textbf{Three observer positions, not two.} ``Overheard the correction'' and ``did
  ...
  \item \textbf{Search over formulas, not just checking of them.} \emph{What does the nurse
    believe that is false?} must be answerable without the analyst guessing the formula
    first (\Cref{sec:query}).
\end{enumerate}

Those five are the paper's design choices, and each is now motivated by something the
scenario needs and an existing approach cannot supply. The remaining sections deliver them in
that order.
...
with the true one, so the goal of \Cref{lst:handover} is unreachable and the capability of
\Cref{sec:running} cannot be discharged at all.
...
\label{ex:ask}
This is the fifth demand of \Cref{sec:demands}, discharged on the scenario that raised it. The
