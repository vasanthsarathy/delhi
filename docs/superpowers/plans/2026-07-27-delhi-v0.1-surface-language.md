...
fn resolve_args(
    ...
    for a in &term.args {
        match a {
            // Checked here rather than left to `sig.atom_id`: for a *constant*
            // predicate the fold short-circuits before that lookup, so a typo'd
            // object would otherwise vanish into a silent `false`.
            Arg::Obj(o) if !sig.objects.contains_key(o) => {
                diags.push(term.span, format!("no object `{o}`"));
                return None;
            }
...
pub fn lower_formula(
    ...
    match e {
        ...
        Expr::Atom(term) => {
            ...
            // `lookup` returning `None` means "declared predicate, this instance was
            // never listed" — which is `false`, silently. A `constants` section
            // idiomatically lists only what is true, and Task 8 grounds an action over
            // every object tuple, so diagnosing the unlisted case would flood any
            // ordinary domain with errors on every run.
            if consts.is_constant_pred(&term.pred) {
                return match consts.lookup(&term.pred, &args) {
                    Some(true) => store.tru(),
                    Some(false) | None => store.fls(),
                };
            }
...
Execution surfaced defects in this document. They are recorded here rather than
silently patched into the task bodies, because the pattern matters more than any
individual fix: **the production code specified here was almost always right, and the
tests specified here often could not have caught it being wrong.** Seven of the fifteen
tasks turned up an assertion that passed against the very bug it named.

### Corrections to the specified code

- **Task 2, the lexer decoded characters with `let c = b[i] as char`.** That is a
  leading-byte cast: for `¿` (`0xC2 0xBF`) it yields `Â`, which never matches the `'¿'`
  arm — so the reference lexer could not lex its own headline operators, and the
  mis-decoded width could then slice mid-character inside `Diagnostics::render`. Replaced
  with `src[i..].chars().next()`.

- **Task 7, an unlisted instance of a declared constant predicate raised a diagnostic
  before folding to false.** That rejects the idiomatic encoding — a `constants` section
  lists only what is true, and Task 8 grounds an action over every object tuple, so every
  non-adjacent pair would error on every run. The rule is now silent-false, matching the
  composition Task 6 already committed to. `resolve_args` gained a `&Sig` parameter and now
  validates object names, which is the one thing the removed diagnostic incidentally
  covered — and covers it better, since `lookup` cannot distinguish a typo from a
  legitimately-undeclared pair.

- **Task 9, the entry classifier rejected `!B[a] q` and `!(p & q)`** as "a fact must be a
  literal", though both hold in the state it builds; and it was inconsistent, since `p & q`
  was already assertion-only and only its negation errored. Each entry was also lowered
  twice, producing duplicate diagnostics. Both fixed.

- **Task 9, `MAX_UNCERTAIN_ATOMS = 16` did not bound what it claimed.** The dominant cost
  is the `O(4^|U|)` relation loop and `n_agents × 2^|U| × 2^|U|` bits — at 16 that is 65,536
  worlds, ~0.5 GB of relation per agent, and ~4.3×10⁹ iterations. A 13-atom input had to be
  killed by a 240-second timeout. Now 12: 4,096 worlds, ~2 MB per agent, ~1.7×10⁷ iterations.

### Corrections to the specified tests

Each of these passed against the bug it named, confirmed by sabotage:

- **Task 5** asserted `is_subtype("Robot", "Object")` for transitivity, but `is_subtype`
  short-circuits `Object` to true. A parent-only implementation passed all eight tests.
  Replaced with a three-level chain.
- **Task 6** named a test for an unlisted instance of a constant predicate but checked
  wrong-arity and unknown-predicate instead — and the fixture blanketed every pair, so the
  case did not exist in any test's data.
- **Task 9** asserted `n_worlds == 4`. A collapsed enumeration where every uncertain atom
  reads the same mask bit yields four worlds that validate, with the correct designated
  valuation. Now asserts valuations.
- **Task 11** asserted the output lacks `"u ~ u"`, but the printer renames worlds to `w0`,
  `w1`, so the string could never appear either way.
- **Task 12** asserted only `s3.entails(goal)`. Both conjuncts were already pinned
  separately above, so *any weaker goal* passed — with a conjunct dropped from the file, all
  five tests stayed green. Now pins the goal's `FormulaId`, which hash-consing makes exact
  structural identity.
- **Task 14**'s four `dot` assertions could not detect reflexive self-loops being drawn.
  Since `Model::validate` requires reflexivity everywhere, every output has self-edges
  available to be drawn wrongly.

### A defect in already-approved code, found two tasks later

**Task 4's `causes` clause swallowed the head of the following clause.** `causes p, a
observes` ate the comma unconditionally, read `a` as a second literal — a bare identifier is
a valid zero-arity literal — and left `observes` dangling. Task 4 was reviewed and approved
with this in it, because none of its tests wrote a `causes` list followed by another clause.
Two of Task 8's fixtures could not have parsed.

The fix added a boundary check with two branches. The `observes`/`aware` lookahead is
provably exact. The clause-keyword branch was a heuristic that silently changed valid input:
a proposition named `pre` used as a non-first `causes` literal would misparse. Rather than
tune the heuristic, **the seven clause words — `actor`, `pre`, `causes`, `determines`,
`announces`, `observes`, `aware` — are now reserved and rejected as proposition names**,
which makes the branch exact. That is a language rule this document did not originally have.

### Corrections to prose

- Task 14's hand-check predicted "a four-world state" after three actions on the example
  file. It is sixteen, which agrees with the verified gate trace 2 → 4 → 8 → 16.
- Task 8's prose calls `observer_condition` six parameters; it is seven, which sits *at*
  clippy's threshold rather than above it, so no `allow` is needed.
- The borrow-discipline note repeated in several tasks — that `p.actions.iter().find(…)`
  borrows the whole `Problem` — is wrong. Field borrows are disjoint; only the whole-self
  method `p.action(name)` forces the clone. The clones are still right as convention, but
  not because the compiler demands them.
